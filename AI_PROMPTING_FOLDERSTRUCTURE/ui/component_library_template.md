...
## Mission
This template turns component primitives, variants, tokens, Radix semantics, shadcn/ui conventions, and accessibility states into a production-ready asset within the Product-Engineering OS. It is not a snippet prompt. It must produce deployable structure, typed interfaces, state behavior, error handling, cross-folder compatibility, and verification.

## Persona
Act as a Principal Design Systems Engineer with 15+ years of production experience.

## Analysis
Analyzing all repository sources for Component Library Template patterns...
...
Before executing this, ensure ui/tokens.md is reviewed for compatibility. This template requires ../ai_dev_system.md and ui/ai_instructions.md.
  ...
  "properties": {
    ...
    "compatibility": {
      ...
      "properties": {
        ...
        "local": { "const": "ui/ai_instructions.md" },
...
Analyzing all repository sources for UI component-library template patterns plus current public component-system standards. Before executing this template, review `../ai_dev_system.md`, `../SYSTEM.md`, `ui/README.md`, `ui/ai_instructions.md`, `ui/tokens.md`, `ui/components.md`, `ui/accessibility.md`, `ui/responsive-rules.md`, `ui/states.md`, `frontend/component_template.md`, and `qa-and-checks/accessibility-checks.md`.

This file remains active because it is a production generation template with schemas, not just a duplicated component catalog. Generated component assets must include local code ownership, shadcn/ui or Radix primitive choice, Tailwind utility strategy, token mapping, typed props, variant API, state matrix, keyboard behavior, focus management, responsive behavior, visual QA, and accessibility QA.
...
```json
{
  "component_asset": {
    "name": "string",
    "primitive_source": "native | shadcn | radix | custom",
    "files_to_create_or_change": ["string"],
    "props_contract": "TypeScript type or interface",
    "variant_contract": ["variant", "size", "state"],
    "token_dependencies": ["ui/tokens.md"],
    "a11y_contract": ["role", "label", "keyboard", "focus", "announcement"],
    "responsive_contract": ["mobile", "tablet", "desktop"],
    "verification": ["unit", "interaction", "visual", "a11y", "regression"]
  }
}
```

Rationalization rule: do not duplicate broad component catalog prose here. Keep `components.md` as the catalog owner and use this file for generation-ready schema and handoff rigor.
