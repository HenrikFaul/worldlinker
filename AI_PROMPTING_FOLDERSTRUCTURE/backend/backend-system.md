...
## Analysis
Analyzing all repository sources for Backend System patterns...
...
## Purpose
Define backend ownership, controller/service boundaries, API envelopes, auth checks, logging, and non-regression rules.
...
`json
{
  ...
  "cross_folder_dependencies": ["backend/ai_instructions.md; database/schema_template.md; integration/api_client_template.md"],
  "interfaces": {
    "json_schema": {},
...
- Before executing this, ensure backend/ai_instructions.md; database/schema_template.md; integration/api_client_template.md is reviewed for compatibility.
...
- backend/ai_instructions.md; database/schema_template.md; integration/api_client_template.md
...
### Analysis
Analyzing all repository sources for Backend System supadupa-enhancement patterns...

This enhancement layer deepens the existing Backend artifact with the newest repository-wide operating memory. It preserves all prior content and adds stronger instructions for source ingestion, append-only governance, production-ready asset generation, and QA-verifiable delivery.

### Added Capability
Adds contract registry, exception taxonomy, and data-integrity QA dependency.
...
~~~json
{
  ...
  "non_regression": true,
  "append_only": true,
  "compatibility_links": ["docs/interface-contract-registry.md; qa-and-checks/data-integrity-checks.md"],
  "verification_required": ["reference check", "schema/interface check", "edge-state check", "qa handoff check"]
}
~~~

Before executing this enhancement, ensure docs/interface-contract-registry.md; qa-and-checks/data-integrity-checks.md are reviewed for compatibility.

<!-- ENHANCEMENT-PASS-2026-05-07-BACKEND-DATABASE-NO-NEW-FILES -->

## 2026-05-07 Backend System Upgrade

This append-only upgrade turns the backend system into a stricter execution environment for production assets. It adds no new files and preserves every prior decision.

Backend ownership now requires a DB-first readiness check before implementation: schema invariants, RLS/tenant model, migrations, generated types, and query performance must be confirmed or documented as assumptions. Controller and service assets must expose request validation, service orchestration, API envelope, idempotency rule, auth source, error taxonomy, observability fields, timeout budget, retry classification, and QA evidence. Any backend artifact that touches UI-facing behavior must also hand off loading, empty, error, conflict, permission, stale-data, and success states to frontend/UI/UX owners.

Required cross-checks: `database/schema-rules.md`, `database/migrations.md`, `database/performance.md`, `backend/controllers.md`, `backend/services.md`, `backend/exceptions.md`, `integration/api-contracts.md`, and `qa-and-checks/security-checks.md`.

<!-- ENHANCEMENT-PASS-2026-05-06-REPOSITORY: full companion content appended verbatim from backend-system.repository-enhancement-20260506.md -->

# Backend System - Repository Enhancement 2026-05-06

## Scope
Companion enhancement for `backend/backend-system.md`. It deepens architecture rules without removing prior backend decisions.

## Analysis
Analyzing all repository sources for backend architecture scope, module boundaries, route isolation, and production diagnostics patterns...

The repository shows a diagnostics-heavy backend culture: API key validation, request tracing, Supabase function inventory, SSRF guards, redacted probes, timeout caps, no-store requests, and explicit auth/no-auth comparisons. It also preserves enterprise runtime semantics such as RLS, approval math, quota math, schema split, transactional email registry, feature flags, and auditability.

## Enhancement Rules
- A backend module is production-ready only when it declares route ownership, service ownership, data ownership, provider dependencies, auth model, observability events, and rollback behavior.
- Routes must be thin: parse, authenticate, authorize, validate, call service, normalize response. Business logic belongs in services.
- Services must be idempotent where possible and must declare side-effect ordering, transaction boundaries, retry policy, and timeout budget.
- Diagnostics routes must block SSRF/private-network targets unless explicitly safe, redact secrets, cap redirects, cap total duration, and return evidence rather than optimistic claims.
- Provider import routes must stamp current load sessions, verify source count against target count, fail on duplicate provider keys, and retry deltas rather than replaying all data blindly.
- Backend modules must never silently replace existing notification, audit, permission, or feature-flag systems.

## Contract
Before executing this, ensure `backend/controllers.md`, `backend/services.md`, `database/performance.md`, and `integration/api-contracts.md` are reviewed for compatibility.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "BackendSystemOutput",
  "type": "object",
  "required": ["routes", "services", "data_dependencies", "observability", "failure_modes", "tests"],
  "properties": {
...
```ts
type BackendSystemStatusProps = {
  routes: Array<{ path: string; method: string; auth: "public" | "user" | "service" }>;
...
## Verification
Confirm route, service, integration, database, security, and QA references resolve before marking a backend system asset complete.
