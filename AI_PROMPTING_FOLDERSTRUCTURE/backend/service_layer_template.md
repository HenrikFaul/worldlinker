...
## Mission
This template turns domain orchestration, transaction boundaries, idempotency, retries, rollback, and service decomposition into a production-ready asset within the Product-Engineering OS. It is not a snippet prompt. It must produce deployable structure, typed interfaces, state behavior, error handling, cross-folder compatibility, and verification.

## Persona
Act as a Principal Backend Service Architect with 15+ years of production experience.

## Analysis
Analyzing all repository sources for Service Layer Template patterns...
...
## 2026-05-07 Service Template Upgrade

The service template must produce an executable-quality service blueprint: command/query name, input contract, output contract, invariants, transaction boundary, auth/tenant verification, repository calls, external calls, retry/idempotency behavior, domain events, exception mapping, logging/redaction, and tests.

Require explicit handling for stale records, duplicate requests, RLS denial, FK integrity failures, nullable relations, generated type drift, provider throttling, empty results, and partial writes. Any SQL-dependent assumption must link back to database schema and performance artifacts. Any UI-facing outcome must include UX state handoff copy.

Cross-check before use: `database/schema-rules.md`, `database/migrations.md`, `database/performance_tuning.md`, `backend/tests.md`, and `qa-and-checks/data-integrity-checks.md`.

<!-- ENHANCEMENT-PASS-2026-05-06-REPOSITORY: full companion content appended verbatim from service_layer_template.repository-enhancement-20260506.md -->

# Service Layer Template - Repository Enhancement 2026-05-06

## Scope
Companion enhancement for `backend/service_layer_template.md`.

## Analysis
Analyzing all repository sources for business logic assets with database compatibility, external provider safety, and retry discipline...

The service layer must be the place where repo lessons become reliable execution: idempotent imports, source evidence, typed errors, transaction scope, and current-session verification.

## Required Service Template Sections
- Purpose and domain boundary.
- Inputs and JSON Schema.
- Outputs and TypeScript result type.
- Dependencies: database tables, provider clients, feature flags, queue jobs, and integration contracts.
- Idempotency: key choice, conflict behavior, replay behavior, and duplicate-source failure.
- Transactions: isolation needs, rollback behavior, chunking strategy, and lock-risk budget.
- Observability: correlation ID, metrics, redacted logs, provider evidence, and run summaries.
- Tests: pure logic, mocked provider, database integration, RLS, retry exhaustion, partial success, and rollback.

## Contract
Before executing this, ensure `backend/services.md`, `database/migrations.md`, and `database/performance.md` are reviewed for compatibility.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "ServiceLayerAsset",
  "type": "object",
  "required": ["purpose", "inputs", "outputs", "idempotency", "transactions", "observability", "tests"],
  "properties": {
    "purpose": { "type": "string" },
    "inputs": { "type": "object" },
    "outputs": { "type": "object" },
...
## Verification
Generated services must prove safe rerun behavior and explain how database constraints protect against duplicates or stale success.
