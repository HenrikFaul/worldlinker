...
## Mission
This template turns secure JSON APIs, request validation, response envelopes, permission-aware behavior, and operational telemetry into a production-ready asset within the Product-Engineering OS. It is not a snippet prompt. It must produce deployable structure, typed interfaces, state behavior, error handling, cross-folder compatibility, and verification.

## Persona
Act as a Principal Backend API Architect with 15+ years of production experience.

## Analysis
Analyzing all repository sources for Backend API Prompt Template patterns...
...
Before executing this, ensure database/schema_template.md is reviewed for compatibility. This template requires ../ai_dev_system.md and backend/ai_instructions.md.
  ...
  "properties": {
    ...
    "compatibility": {
      ...
      "properties": {
        ...
        "local": { "const": "backend/ai_instructions.md" },
...
## 2026-05-07 API Template Upgrade

Every backend API template must require an endpoint contract with route, method, auth guard, tenant boundary, request schema, response schema, error schema, idempotency behavior, pagination/sorting/filtering rules, cache behavior, and observability fields. For mutation APIs, include duplicate prevention and rollback/reconciliation. For read APIs, include empty state semantics, stale-data handling, and performance budget.

The template must explicitly map backend domain outcomes to frontend-visible states: loading, empty, success, validation error, permission error, conflict, retryable failure, and terminal failure. Contract tests are mandatory whenever API envelope or schema changes.

Cross-check before use: `backend/controllers.md`, `backend/services.md`, `backend/exceptions.md`, `database/performance.md`, and `integration/api-contracts.md`.

<!-- ENHANCEMENT-PASS-2026-05-06-REPOSITORY: full companion content appended verbatim from prompt_template_backend_api.repository-enhancement-20260506.md -->

# Backend API Prompt Template - Repository Enhancement 2026-05-06

## Scope
Companion enhancement for `backend/prompt_template_backend_api.md`.

## Analysis
Analyzing all repository sources for API generation with JSON Schema, Supabase auth, RLS compatibility, and evolution-safe response contracts...

API prompts must now force generated assets to include endpoint behavior, contracts, observability, source evidence, and non-regression checks.

## Required API Prompt Output
- Route: method, path, runtime, auth mode, cache behavior, and ownership.
- Request schema: JSON Schema with examples and invalid cases.
- Response schema: stable envelope with normalized arrays.
- Service calls: named service functions, idempotency keys, transaction scope, and provider adapters.
- Database assumptions: tables, RLS policies, indexes, generated types, and migration references.
- Errors: validation, auth, forbidden, RLS denial, provider timeout, rate limit, duplicate key, partial ETL, and internal error.
- Tests: contract, unit, integration, RLS, performance, and rollback verification.

## Contract
Before executing this, ensure `backend/controllers.md`, `backend/services.md`, and `database/schema-rules.md` are reviewed for compatibility.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "BackendApiPromptOutput",
  "type": "object",
  "required": ["route", "request_schema", "response_schema", "service_plan", "error_matrix", "tests"],
  "properties": {
    "route": { "type": "object" },
    "request_schema": { "type": "object" },
    "response_schema": { "type": "object" },
    "service_plan": { "type": "array", "items": { "type": "string" } },
    "error_matrix": { "type": "array", "items": { "type": "string" } },
...
## Verification
The API prompt is incomplete if it permits code without schema, tests, security assumptions, rollback, or source-file evidence.
