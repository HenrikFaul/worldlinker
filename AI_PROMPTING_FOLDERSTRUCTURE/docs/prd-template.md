...
## Analysis
Analyzing all repository sources for PRD Template Hyphen Alias patterns...
...
## Persona
Act as a senior Docs owner with 15+ years of production experience. You are accountable for business behavior, technical correctness, accessibility, security, performance, observability, and QA handoff.

## Purpose
Create hyphenated PRD alias matching my-ai-dev-system map while preserving existing underscore template.
...
`json
{
  ...
  "cross_folder_dependencies": ["docs/prd-template.md; ux/user-flows.md"],
  "interfaces": {
    "json_schema": {},
...
- Before executing this, ensure docs/prd-template.md; ux/user-flows.md is reviewed for compatibility.
...
- docs/prd-template.md; ux/user-flows.md

<!-- LOSSLESS-CONSOLIDATION-IMPORT: AI_PROMPTING_FOLDERSTRUCTURE/docs/prd-template.md -->

## Lossless Consolidation Import From `AI_PROMPTING_FOLDERSTRUCTURE/docs/prd-template.md`
...
Before executing this, ensure ux/user_journey_template.md is reviewed for compatibility. Also read ../ai_dev_system.md and docs/ai_instructions.md.

## Required Contract
~~~json
{
  "owner": "docs",
  "focus": "product requirements, scope, acceptance, analytics, risks, rollout, and stakeholder decisions",
  "source_files_analyzed": ["string"],
  "inputs": {},
  "outputs": {},
  "interfaces": {
  ...
  "edge_cases": ["string"],
  "verification": ["string"],
  "compatibility": ["../ai_dev_system.md", "docs/ai_instructions.md", "ux/user_journey_template.md"]
}
~~~
...
## Productized Delivery And Reconstructability Layer

Analyzing all repository sources for product documentation patterns... A PRD must be strong enough to rebuild the product context after memory loss, team change, or repository migration. It must capture product intent, current capabilities, preserved functionality, new value, data ownership, release evidence, and operational rollback in one place without duplicating implementation files.

Required PRD additions:
- Product identity: product name, target users, tenant/workspace model, primary jobs-to-be-done, current capability inventory, and explicit non-goals.
- Non-regression inventory: list existing flows, routes, data tables, integrations, reports, permissions, and automation behavior that must remain working.
- Rationalization map: identify overlapping modules, old/new route aliases, duplicated settings, duplicated reports, duplicated prompts, and the chosen canonical owner.
- Help/manual plan: define help anchors, searchable manual sections, contextual help entry points, update owner, and stale-help detection.
- Localization plan: define supported locales, translation namespaces, fallback language, language selector placement, email/deep-link copy ownership, and validation-copy ownership.
- Versioning plan: each major delivery should have a machine-readable release ID, markdown release notes, and a human-readable export artifact when the project expects stakeholder review.
- Feature-flag and rollout plan: define default-off/default-on behavior, staged rollout cohorts, rollback switch, data migration dependencies, and monitoring metrics.

PRD completeness contract:
```json
{
  "prd_contract": {
    "capability_inventory_complete": true,
    "non_regression_inventory_complete": true,
    "canonical_owner_for_duplicates_defined": true,
    "help_manual_plan_defined": true,
    "localization_plan_defined": true,
    "feature_flags_defined": true,
    "release_reconstructability": {
      "release_id": "string",
      "markdown_notes": true,
      "stakeholder_export_when_needed": true
    }
  }
}
```

Compatibility cross-links: before finalizing a PRD, review `../SYSTEM.md`, `../ai_dev_system.md`, `arch-diagrams.md`, `../frontend/project-structure.md`, `../ux/design-system.md`, `../integration/api-contracts.md`, `../database/schema-rules.md`, and `../qa-and-checks/checklist.md`.

<!-- DOC-CREATION-HARMONIZATION-20260510 -->

## Documentation Generation Interoperability Layer

This file owns PRD governance inside the Product-Engineering OS. It does not own generated target-repository user manuals or help-menu manifests.

When full repository documentation must be generated, start from [`../../doc creation/SYSTEM.md`](<../../doc creation/SYSTEM.md>). That documentation-generation pack must read this PRD template when it creates or refreshes:
...
PRD-to-doc-generation requirements:

- Product scope, non-goals, capability inventory, rollout, rollback, help/manual plan, localization, and release reconstructability must influence generated docs.
- Generated user manuals must not invent product behavior; they must trace workflow claims back to current repository evidence.
- Generated feature catalogs must map product capabilities to routes, backend behavior, entities, tests, or explicit evidence gaps.
- Generated documentation reports must record PRD gaps, stale requirements, and missing acceptance evidence.
- All links must be repository-relative and portable across GitHub repositories.

Archived underscore alias note: `prd_template.md` is no longer an active owner file after docs harmonization. Its content is preserved in this canonical `prd-template.md` and in `archive/2026-05-10-docs-harmonization/prd_template.md`.
