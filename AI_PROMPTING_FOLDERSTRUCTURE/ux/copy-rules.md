...
## Analysis
Analyzing all repository sources for UX Copy Rules patterns...
...
## Purpose
Define CTA naming, labels, helper text, confirmation copy, destructive copy, localization readiness, and no-blame errors.
...
`json
{
  ...
  "cross_folder_dependencies": ["ux/messaging.md; ui/components.md"],
  "interfaces": {
    "json_schema": {},
...
- Before executing this, ensure ux/messaging.md; ui/components.md is reviewed for compatibility.
...
- ux/messaging.md; ui/components.md

<!-- LOSSLESS-CONSOLIDATION-IMPORT: AI_PROMPTING_FOLDERSTRUCTURE/ux/microcopy_guide.md -->

## Lossless Consolidation Import From AI_PROMPTING_FOLDERSTRUCTURE/ux/microcopy_guide.md

This imported source remains fully preserved here so the active UX copy contract can replace the legacy standalone microcopy guide without content loss.

# Microcopy Guide

## Purpose
This guide defines production-grade rules for labels, CTAs, errors, empty states, confirmations, localization-ready copy, and recovery language inside the Product-Engineering OS.

## Analysis
Analyzing all repository sources for Microcopy Guide patterns...
...
Before executing this, ensure UI Masterprompt.md is reviewed for compatibility. Also read ../ai_dev_system.md and ux/ai_instructions.md.

## Required Contract
~~~json
{
  "owner": "ux",
  "focus": "labels, CTAs, errors, empty states, confirmations, localization-ready copy, and recovery language",
  "source_files_analyzed": ["string"],
  "inputs": {},
  "outputs": {},
  "interfaces": {
  ...
  "edge_cases": ["string"],
  "verification": ["string"],
  "compatibility": ["../ai_dev_system.md", "ux/ai_instructions.md", "UI Masterprompt.md"]
}
~~~
...
Analyzing all repository sources for UX copy patterns plus current public UX standards. Before executing this file, review `../ai_dev_system.md`, `../SYSTEM.md`, `ux/README.md`, `ux/messaging.md`, `ux/states.md`, `ui/accessibility.md`, and `qa-and-checks/checklist.md`.

Copy must be production-ready, not placeholder prose. Use action verbs for CTAs, specific nouns for objects, and direct recovery language for errors. Avoid internal implementation terms, ambiguous labels such as "Submit" when the action is concrete, and clever wording that reduces scannability. Every field must define label, helper text when needed, validation timing, error message, success/confirmation text, and localization notes.

Copy contract:
```json
{
  "surface": "page | modal | form | wizard | search | notification",
  "primary_cta": "verb + object",
  "secondary_cta": "safe alternative",
  "destructive_cta": "explicit consequence",
  "empty_state": {
    "title": "what is missing",
    "body": "why it matters",
    "action": "next useful step"
  },
  "error_state": {
    "problem": "plain-language diagnosis",
    "recovery": "specific action",
    "fallback": "support or retry path"
  },
  "localization": ["no idioms", "variables documented", "plural rules considered"]
}
```

Rationalization rule: `microcopy_guide.md` is now losslessly represented in this file. Do not create a new standalone microcopy guide; append new reusable wording systems here and cross-link conceptual tone changes back to `ux/messaging.md`.
