# Wizard Flow Template

## Mission
This template turns multi-step flows, branching, validation, progress, abandonment recovery, and responsive/accessibility behavior into a production-ready asset within the Product-Engineering OS. It is not a snippet prompt. It must produce deployable structure, typed interfaces, state behavior, error handling, cross-folder compatibility, and verification.

## Persona
Act as a Senior Product Flow Architect with 15+ years of production experience.

## Analysis
Analyzing all repository sources for Wizard Flow Template patterns...
...
Before executing this, ensure ui/accessibility_rules.md is reviewed for compatibility. This template requires ../ai_dev_system.md and ux/ai_instructions.md.
...
Analyzing all repository sources for wizard-flow patterns plus current public UX standards. Before executing this file, review `../ai_dev_system.md`, `../SYSTEM.md`, `ux/README.md`, `ux/user-flows.md`, `ux/states.md`, `ux/messaging.md`, `ux/copy-rules.md`, `ui/accessibility.md`, `frontend/routing.md`, and `qa-and-checks/checklist.md`.

Wizard flows must reduce cognitive load while preserving user control. Define entry criteria, step order, progress feedback, optional/skippable steps, validation timing, back/forward behavior, save-and-resume, cancellation, summary/review, confirmation, and recovery from network or permission failures. Each step must have one primary intent and must avoid hiding critical consequences until the end.
  ...
  "steps": [
    {
      ...
      "validation": "sync | async | deferred",
      "states": ["loading", "invalid", "valid", "saving", "error"],
      "exit_paths": ["next", "back", "cancel", "save_resume"]
    }
  ],
  "recovery": ["retry", "save_draft", "contact_support", "manual_alternative"],
  "verification": ["keyboard", "focus_order", "mobile", "partial_completion", "error_recovery"]
}
```

Rationalization rule: keep this file active because wizards need deeper sequencing than general journey maps. Reference `ux/user-flows.md` for upstream/downstream journey context and `ux/states.md` for per-step behavior.
