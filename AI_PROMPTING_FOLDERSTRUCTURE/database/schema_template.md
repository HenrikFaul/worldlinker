...
## Mission
This template turns schema design, constraints, indexes, durable invariants, data lifecycle, and compatibility into a production-ready asset within the Product-Engineering OS. It is not a snippet prompt. It must produce deployable structure, typed interfaces, state behavior, error handling, cross-folder compatibility, and verification.

## Persona
Act as a Principal Database Architect with 15+ years of production experience.

## Analysis
Analyzing all repository sources for Schema Template patterns...
...
Before executing this, ensure docs/prd_template.md is reviewed for compatibility. This template requires ../ai_dev_system.md and database/ai_instructions.md.
...
## 2026-05-07 Schema Template Upgrade

The schema template must output production-ready data contracts, not just DDL. Require entity purpose, ownership, lifecycle, tenant boundary, PII class, retention rule, column semantics, nullable rules, constraints, FK behavior, indexes, RLS policy model, generated type impact, API/DTO impact, seed examples, and performance expectations.

For each relation, include cardinality, delete/update behavior, access visibility under RLS, Supabase client type shape, and downstream DTO mapping. For each enum or status, include allowed transitions and stale/conflict handling. For each query-critical column, include expected selectivity and index justification.

Cross-check before use: `database/schema-rules.md`, `database/performance.md`, `backend/services.md`, and `docs/interface-contract-registry.md`.

<!-- LOSSLESS-CONSOLIDATION-IMPORT: database/schema_template.repository-enhancement-20260506.md -->

## Lossless Consolidation Import From `database/schema_template.repository-enhancement-20260506.md`
...
### Source Scope
Companion enhancement for `database/schema_template.md`.

### Source Analysis
Analyzing all repository sources for schema asset generation covering canonical tables, provider registries, external events, and master data patterns...

The canonical address and external-events SQL establish reusable schema motifs: merge keys, provider source identity, raw payloads, active flags, timestamps, generated values, comments, RLS, and focused indexes.

### Source Required Schema Template Sections
- Entity purpose and lifecycle.
- Source authority and provenance.
- SQL DDL with comments.
...
### Source Contract
Before executing this, ensure `database/schema-rules.md`, `database/performance.md`, and `integration/api-contracts.md` are reviewed for compatibility.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "SchemaTemplateOutput",
  "type": "object",
  "required": ["ddl", "constraints", "rls", "indexes", "typescript_interface", "validation"],
  "properties": {
    "ddl": { "type": "string" },
...
```ts
type SchemaTemplateReviewProps = {
  entityName: string;
...
### Source Verification
Schema output is incomplete unless comments, indexes, constraints, RLS, interfaces, seed behavior, and validation SQL are all present.
