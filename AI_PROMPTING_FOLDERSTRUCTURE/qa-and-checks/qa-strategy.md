...
## Analysis
Analyzing all repository sources for QA Strategy patterns...
...
## Purpose
Define TDD/BDD, characterization tests, contract tests, visual QA, accessibility QA, and release confidence model.
...
`json
{
  ...
  "cross_folder_dependencies": ["qa-and-checks/checklist.md; qa/unit_test_template.md"],
  "interfaces": {
    "json_schema": {},
...
- Before executing this, ensure qa-and-checks/checklist.md; qa/unit_test_template.md is reviewed for compatibility.
...
- qa-and-checks/checklist.md; qa/unit_test_template.md

<!-- QA-INTERNET-TUNED-RATIONALIZATION-2026-05-10 -->

## QA Rationalization And Verification Layer

Analyzing all repository sources for QA rationalization patterns... This is the canonical QA strategy for release confidence. `qa/` remains a specialist test-template library, while `qa-and-checks/` owns the release gate. `qa/qa_checklists.md` is consolidated into `checklist.md` and archived. Before executing this, ensure `qa-and-checks/README.md`, `checklist.md`, and the relevant `qa/` template are reviewed for compatibility.

External standards added in this pass: Playwright best practices, WCAG 2.2, Core Web Vitals, OWASP ASVS, OWASP Top 10, and test-pyramid practice.

Required strategy contract:
~~~json
{
  "coverage_shape": { "unit": "broad", "contract": "required", "integration": "boundary-focused", "e2e": "critical-flow-only" },
  "release_gates": ["accessibility", "security", "performance", "data-integrity", "integration-resilience", "visual-regression"],
  "evidence_mode": "runtime-observed | code-inferred | documented-assumption",
  "blocking_conditions": ["missing source traceability", "untested critical path", "unreviewed security impact", "unbounded performance risk"]
}
~~~
