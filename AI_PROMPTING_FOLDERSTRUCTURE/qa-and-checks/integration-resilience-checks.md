...
## Analysis
Analyzing all repository sources for Integration Resilience Checks patterns...
...
## Purpose
Define third-party API resilience checks for timeouts, SSRF protection, redaction, provider validation, webhooks, retries, and rate limits.
...
6. integration/api-contracts.md; integration/error-handling.md

## Input Contract
~~~json
{
  "task_id": "string",
  "artifact": "Integration Resilience Checks",
  ...
  "cross_links_reviewed": ["integration/api-contracts.md; integration/error-handling.md"],
  "interfaces": {
    "json_schema": {},
...
Before executing this, ensure these files are reviewed for compatibility: integration/api-contracts.md; integration/error-handling.md.
...
Analyzing all repository sources for QA rationalization patterns... This file remains the specialist integration-resilience gate under the canonical `qa-and-checks/` release surface. Before executing this, ensure `integration/README.md`, `integration/api-contracts.md`, `integration/error-handling.md`, `integration/webhook_handler.md`, `qa-and-checks/security-checks.md`, and `qa-and-checks/checklist.md` are reviewed for compatibility.

Added gate: distinguish HTTP success from business success, verify timeout caps, retry/backoff policy, rate-limit handling, SSRF/private-network blocking, redaction, correlation IDs, idempotency keys, webhook signature/replay protection, and provider-specific validation.
