...
## Analysis
Analyzing all repository sources for Security Checks patterns...
...
## Persona
Act as a senior Security QA owner with 15+ years of production experience. You are accountable for business behavior, technical correctness, accessibility, security, performance, observability, and QA handoff.

## Purpose
Define auth, secrets, SSRF, redaction, rate limit, RLS, webhook signature, dependency, and logging security checks.
...
`json
{
  ...
  "cross_folder_dependencies": ["integration/auth-flows.md; backend/exceptions.md"],
  "interfaces": {
    "json_schema": {},
...
- Before executing this, ensure integration/auth-flows.md; backend/exceptions.md is reviewed for compatibility.
...
- integration/auth-flows.md; backend/exceptions.md

<!-- QA-INTERNET-TUNED-RATIONALIZATION-2026-05-10 -->

## QA Rationalization And Verification Layer

Analyzing all repository sources for QA rationalization patterns... This file remains the specialist security gate under the canonical `qa-and-checks/` release surface. Before executing this, ensure `qa-and-checks/README.md`, `checklist.md`, `integration/auth-flows.md`, `integration/error-handling.md`, `backend/exceptions.md`, and `database/schema-rules.md` are reviewed for compatibility.

OWASP ASVS and Top 10 additions: verify authentication, authorization, session lifecycle, input validation, SSRF/private-network defense, unsafe API consumption, dependency risk, secrets handling, logging redaction, rate limiting, CSRF where applicable, webhook signature verification, and safe user-facing error messages.
