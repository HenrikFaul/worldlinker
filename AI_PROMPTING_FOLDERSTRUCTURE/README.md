...
External coding AIs should be given **one starting file only**:

`AI_PROMPTING_FOLDERSTRUCTURE/SYSTEM.md`

`SYSTEM.md` is the bootstrap controller. It tells the AI to read `build_log.md`, `ai_dev_system.md`, this `README.md`, `.ai/*`, the relevant domain files, and every compatibility/checklist/template file required by the task. Do not manually start a coding AI from `ai_dev_system.md`; that file is loaded second through the `SYSTEM.md` directive.
...
## Analysis
Analyzing all repository sources for Project README patterns...
...
## Persona
Act as a senior Product-Engineering OS owner with 15+ years of production experience. You are accountable for business behavior, technical correctness, accessibility, security, performance, observability, and QA handoff.

## Purpose
Project overview, setup, read order, recursive execution, idempotent rerun rules, and usage instructions.

## Required Input Contract
`json
{
  "task_id": "string",
  "domain": "Product-Engineering OS",
  "source_files_analyzed": ["string"],
  "current_behavior": "string",
  "target_asset": "string",
  "non_regression_constraints": ["string"],
  "cross_folder_dependencies": ["ai_dev_system.md; SYSTEM.md; build_log.md"],
  "interfaces": {
    "json_schema": {},
...
## Operating Rules
- Read `SYSTEM.md` first as the single bootstrap controller; then follow its references to `build_log.md`, `ai_dev_system.md`, docs/repository inventories, and the local domain instruction before execution.
- Before executing this, ensure `SYSTEM.md`, `ai_dev_system.md`, and `build_log.md` are reviewed for compatibility through the `SYSTEM.md` read order.
...
- ai_dev_system.md; SYSTEM.md; build_log.md
...
### Analysis
Analyzing all repository sources for Project README supadupa-enhancement patterns...

This enhancement layer deepens the existing Docs artifact with the newest repository-wide operating memory. It preserves all prior content and adds stronger instructions for source ingestion, append-only governance, production-ready asset generation, and QA-verifiable delivery.

### Added Capability
Adds operator workflow for the new supadupa artifacts and rerun scripts.
...
~~~json
{
  ...
  "non_regression": true,
  "append_only": true,
  "compatibility_links": ["scripts/supadupa_transform_my_ai_dev_system.ps1; build_log.md"],
  "verification_required": ["reference check", "schema/interface check", "edge-state check", "qa handoff check"]
}
~~~

Before executing this enhancement, ensure scripts/supadupa_transform_my_ai_dev_system.ps1; build_log.md are reviewed for compatibility.
